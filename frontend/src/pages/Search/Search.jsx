...
import { searchSongs, searchSemantic, getTrendingSongs } from "../../services/songService";
...
const Search = () => {
  ...
  // Tìm kiếm ngữ nghĩa chạy độc lập với tìm kiếm từ khóa:
  // nếu Gemini lỗi hoặc chậm, không ảnh hưởng đến kết quả từ khóa ở trên.
  useEffect(() => {
    if (!query) {
      setSemanticResults([]);
    ...
    const runSemantic = async () => {
      try {
        ...
        setSemanticFailed(false);
        const data = await searchSemantic(query, 8);
        setSemanticResults(data);
      } catch (err) {
        console.error("Lỗi khi tìm kiếm ngữ nghĩa:", err);
        setSemanticResults([]);
                ...
                <div className="flex flex-col lg:flex-row gap-8">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex flex-col">
                      {results.map((item, index) => renderSongRow(item, index))}
                    </div>
                  </div>
                </div>
              )}

              {/* Gợi ý bằng AI: luôn thử hiển thị, không phụ thuộc vào việc từ khóa có tìm thấy hay không */}
              {!semanticFailed && (semanticLoading || semanticResults.length > 0) && (
                <div className="mt-10">
                  <h2 className="text-xl font-bold text-white mb-1 flex items-center gap-2">
                    Gợi ý theo ngữ nghĩa
                  </h2>
                  <p className="text-[#a7a7a7] text-sm mb-4">
                    Bài hát phù hợp với mô tả "{query}" theo ngữ nghĩa, kể cả khi tên bài hát không trùng từ khóa.
                  </p>
                  {semanticLoading ? (
                    <p className="text-[#a7a7a7] text-sm font-medium">Đang phân tích ngữ nghĩa...</p>
                  ) : (
                    <div className="flex flex-col max-w-2xl">
                      {semanticResults.map((item, index) => renderSongRow(item, index))}
                    </div>
                  )}
                </div>
              )}
